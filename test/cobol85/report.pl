...
my $ebcdicopt;
if ($ENV{'COB_TEST_EBCDIC'}) {
   $ebcdicopt="-febcdic-charset"
}
if ($opt eq "nocomp") {
   $compile = "echo nocomp";
   $compile_module = "echo nocomp";
} else {
      if ($opt) {
         $compile = "cobc -w -std=cobol85 -save-temps $cobol_flags $ebcdicopt -x $opt";
         $compile_module = "cobc -w -std=cobol85 -save-temps $cobol_flags $ebcdicopt -m $opt";
      } else {
           $compile = "cobc -w -std=cobol85 -save-temps $cobol_flags $ebcdicopt -x ";
           $compile_module = "cobc -w -std=cobol85 -save-temps $cobol_flags $ebcdicopt -m";
      }
}
...
foreach $in (sort (glob("*.{CBL,SUB}"))) {
  ...
  if ($skip{$exe} || $exe =~ /^..[34]0/) {
    ...
    $num_progs++;
    $copy = ($exe =~ /^SM/) ? "-I ../copy " : "";
    print "$compile -G $copy$in && $cmd\n";
    if ($in eq "SM206A.CBL") {
      $ret = system ("$compile -G -fdebugging-line $copy $in");
    } else {
      $ret = system ("$compile -G $copy $in");
    }
    if ($ret != 0) {
      $compile_error++;
	...
	my $inspect = 0;
        if ($ENV{'COB_TEST_EBCDIC'}) {
           system ("cp report.log $exe.log.ebcdic");
           system ("uconv report.log -f ibm-1140 > ebcdicconv.txt");
           system ("cp ebcdicconv.txt report.log");
